...
2. In Colab Secrets, add `GITHUB_REPOSITORY`, a read-only `GITHUB_TOKEN` authorized for the private repository, and `HF_TOKEN` (the secret name is exactly `HF_TOKEN`). Never print a secret or place one in a notebook output.
